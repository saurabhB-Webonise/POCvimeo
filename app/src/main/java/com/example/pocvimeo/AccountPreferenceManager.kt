...
object AccountPreferenceManager {
    ...
    var clientAccount: VimeoAccount?
        get() {
            ...
            return VimeoNetworkUtil.getGson().getAdapter(VimeoAccount::class.java).fromJson(accountJSON)
        }
        set(vimeoAccount) {
            if (vimeoAccount != null) {
                VimeoNetworkUtil.getGson().getAdapter(VimeoAccount::class.java).toJson(vimeoAccount)?.also {
    ...
    fun cacheClientCredentialsAccount(vimeoAccount: VimeoAccount?) {
        vimeoAccount?.run {
            val accountJSON = VimeoNetworkUtil.getGson().getAdapter(VimeoAccount::class.java).toJson(vimeoAccount) ?: return
